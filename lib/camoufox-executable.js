...
function ensureLaunchShim(executablePath, resourceDir) {
  ...
  rmSync(shimExecutable, { force: true, recursive: true });
  ...
  for (const name of ['properties.json', 'version.json']) {
    ...
    const linkPath = join(shimRoot, name);
    rmSync(linkPath, { force: true, recursive: true });
    if (existsSync(target)) ensureSymlink(realpathSync(target), linkPath);
  }

  const fontconfig = join(resourceDir, 'fontconfig');
  const fontconfigLink = join(shimRoot, 'fontconfig');
  rmSync(fontconfigLink, { force: true, recursive: true });
  if (existsSync(fontconfig)) ensureSymlink(realpathSync(fontconfig), fontconfigLink, 'dir');
...
function ensureCamoufoxJsCache(resourceDir, cacheDir, executablePath) {
  ...
  const fontconfig = join(resourceDir, 'fontconfig');
  if (!existsSync(versionFile) || !existsSync(propertiesFile)) {
    throw new Error(
      `External Camoufox bundle at ${resourceDir} must include properties.json and version.json for camoufox-js compatibility`
    );
  }

  mkdirSync(cacheDir, { recursive: true });
  rmSync(cacheVersion, { force: true, recursive: true });
  rmSync(cacheProperties, { force: true, recursive: true });
  rmSync(cacheFontconfig, { force: true, recursive: true });
  rmSync(cacheExecutable, { force: true, recursive: true });

  ensureSymlink(realpathSync(versionFile), cacheVersion);
